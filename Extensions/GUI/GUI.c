...
TEXRESULT Destroy_ButtonHeader(RHeaderButton* pResourceHeader, RHeaderButton* pResourceHeaderOverlay, bool Full, uint32_t ThreadIndex) {
	pResourceHeader->Callback_Function = NULL;

	//for full destruct items.
	if ((((pResourceHeaderOverlay != NULL) ? (pResourceHeader->CallbackSymbol != pResourceHeaderOverlay->CallbackSymbol) : false) || Full == true) && pResourceHeader->CallbackSymbol != NULL) {
...
TEXRESULT Destroy_FontHeader(RHeaderFont* pResourceHeader, RHeaderFont* pResourceHeaderOverlay, bool Full, uint32_t ThreadIndex) {
	//if they are different its safe to destruct.
	if (((pResourceHeaderOverlay != NULL) ? (pResourceHeader->FtFace != pResourceHeaderOverlay->FtFace) : true) && pResourceHeader->FtFace != NULL) {
		FT_Error err = 0;
...
void UpdateSignature_Text(GraphicsEffectSignature* pSignature, RHeaderGraphicsWindow* pGraphicsWindow, uint32_t FrameIndex, GPU_Allocation* GPU_Buffers, uint64_t* GPU_BufferPointers, uint32_t ThreadIndex) {
...
TEXRESULT ReCreate_Text(ElementGraphics* pElement, GraphicsEffectText* pEffect, uint32_t ThreadIndex) {
	...
	pEffect->GPU_GraphicsEffectInfosSize = NULL;
	{
		...
		float AdvanceX = 0;
		float AdvanceY = (float)((pEffect->FontSize * Utils.Config.DPI) / 72) / 2;
		...
		for (size_t i = 0; i < size; i++){
			...
			if (CurrentCodePoint == 0x0D || CurrentCodePoint == 0x0A)
			{
				...
				AdvanceX = 0;
				AdvanceY += ((pEffect->FontSize * Utils.Config.DPI) / 72) / 2;
			}
			else {
				bool tab = false;
				...
				if (FT_HAS_FIXED_SIZES(pFont->FtFace)) {
					...
					scalefactor = ((float)((pEffect->FontSize * Utils.Config.DPI) / 72) / (float)(pFont->FtFace->size->metrics.height >> 6)) / 2;
				}
				else {
					if ((FTerr = FT_Set_Char_Size(pFont->FtFace, pEffect->FontSize, pEffect->FontSize,
						((pGraphicsWindow->CurrentExtentWidth * Utils.Config.DPI) >> 6), ((pGraphicsWindow->CurrentExtentWidth * Utils.Config.DPI) >> 6))) != 0)
				...
				if (tab == true) {
					pGPU_effect->AdvanceX = AdvanceX;
					pGPU_effect->AdvanceY = AdvanceY;
					AdvanceX += ((pFont->FtFace->glyph->advance.x >> 6) * scalefactor) * Utils.Config.TabIndents;
				}
				else
				{
					pGPU_effect->AdvanceX = AdvanceX;
...
TEXRESULT Create_Text(ElementGraphics* pElement, GraphicsEffectText* pEffect, GraphicsEffectCreateInfoText* pEffectCreateInfo, uint64_t* pAllocationSize, uint32_t ThreadIndex) {
	...
	else
	{
		...
		pEffect->iFontsSize = pEffectCreateInfo->iFontsSize;
		for (size_t i = 0; i < pEffect->iFontsSize; i++)
		{
			pEffect->iFonts[i] = pEffectCreateInfo->iFonts[i];
		}
		ReCreate_Text(pElement, pEffect, ThreadIndex);
	}
	*pAllocationSize = sizeof(GraphicsEffectText) + (sizeof(*pEffectCreateInfo->iFonts) * pEffectCreateInfo->iFontsSize);
	return (Success);
}
...
TEXRESULT Update_GUI() {
	ElementAllocation ClickedElement = { NULL, NULL };
	ResourceHeaderAllocation ClickedResourceHeader = { NULL, NULL };
	ObjectAllocation ClickedObject = { NULL, NULL };
	
	Check_Click(&ClickedElement, &ClickedResourceHeader, &ClickedObject,
		(uint32_t)EngineRes.pUtils->MousePos_Callback_state.X_Position, (uint32_t)EngineRes.pUtils->MousePos_Callback_state.Y_Position);
	if (Object_Ref_Get_ObjectAllocationData(ClickedObject) != NULL &&
		Object_Ref_Get_ResourceHeaderAllocationData(ClickedResourceHeader) != NULL &&
...
TEXRESULT Initialise_GUI() {
	...
	Utils.TextSig.UpdateSignature = (UpdateSignature_GraphicsEffectTemplate*)&UpdateSignature_Text;
...
__declspec(dllexport) void Initialise_Resources(ExtensionCreateInfo* ReturnInfo) {
	...
	ConfigParameterExport(&ReturnInfo->ConfigParameters, &ReturnInfo->ConfigParametersSize, (const UTF8*)CopyData((void*)"GUI::DPI"), &Utils.Config.DPI, 1, sizeof(Utils.Config.DPI));
	ConfigParameterExport(&ReturnInfo->ConfigParameters, &ReturnInfo->ConfigParametersSize, (const UTF8*)CopyData((void*)"GUI::TabIndents"), &Utils.Config.TabIndents, 1, sizeof(Utils.Config.TabIndents));
