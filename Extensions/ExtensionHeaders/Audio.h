...
typedef enum AudioFormat //big endian is not actually supported so pls dont use
{
...
typedef TEXRESULT(ConvertXtoTEXA)(FileData*, TEXA_HEADER**); //conversion template

typedef TEXRESULT(ConvertTEXAtoX)(TEXA_HEADER*, FileData*); //conversion template
...
{
	RHeaderAudioSource* pAudioSource; //required
...
typedef void(UpdateSignature_AudioEffectTemplate)(struct AudioEffectSignature* pSignature); //index of swapchain image

typedef void(Draw_AudioEffectTemplate)(ElementAudio* pElement, ResourceHeader* pHeader, Object* pGameObject,
	void* pEffect, uint32_t FrameCountMin, uint32_t FrameCountMax, AudioElementUsageFlags Usage);
...
typedef struct AudioUtils
{
	...
	//Signature Buffers
	ResourceHeaderBuffer RHeaderAudioSourceBuffer;
	ElementBuffer ElementAudioBuffer;
