...
typedef void(Button_Callback)(ElementAllocation ClickedElement, ResourceHeaderAllocation ClickedResourceHeader, ObjectAllocation ClickedGameObject, struct RHeaderButton* pButton, GUIButtonCallbackState Action);
...
typedef struct GraphicsEffectCreateInfoText {
	...
	size_t pFontsSize;
	RHeaderFont** pFonts; //font set, collection of fonts to use
...
typedef struct GUIUtils
{
	...
	//Font* DefaultFonts;
	//size_t DefaultFontsSize;

	ResourceHeaderBuffer RHeaderButtonBuffer;
	ResourceHeaderSignature RHeaderButtonSig;

	ResourceHeaderBuffer RHeaderFontBuffer;
